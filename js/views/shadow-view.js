 ...
 */
/*
 * ⚠️ **ويبقى ٣٠ — ولم أرفعه في WS-ST وإن طلب المدى ٣٢–٤٤.**
 *
 *    رفعتُه إلى ٣٦ أوّلَ الأمر ليدخل المدى المطلوب، ثمّ قرأتُ
 *    التعليقَ في منتقي الحجم أسفلَ الملفّ: الافتراضُ نزل من ٤١ إلى
 *    ٣٠ **بطلبٍ صريحٍ من صاحب التطبيق** («خلّي إمكانية تصغير الخطّ
 *    أكتر من كده، ويكون ده الافتراضيّ»).
 *
 *    وطلبٌ قديمٌ صريحٌ لا يُنسَخ بطلبٍ جديدٍ يتكلّم عن **المدى**
 *    عمومًا. والمكسبُ المطلوبُ جاء من مكانه الصحيح: درجاتُ
 *    `--sh-len` (وهي تقديرٌ منّي لا اختيارٌ منه) كانت تهبط بالجملة
 *    المتوسّطة إلى ٢٣٫٤px — فرُفعت. و«M/L/XL» في المنتقي تُعطي
 *    ٣٦ و٤١ و٤٨ لمن أرادها.
 */
...
export async function renderShadow(main, sessionId) {
  ...
  ctx = {
    ...
    /*
     * ⚠️ **وخطُّ صفحة النصّ رومانيٌّ لا مائل** (WS-BK · بند ٤).
     *
     *    كان الافتراضُ `philosopher` — وهو مائلٌ بوزن ٣٠٠. والمائلُ
     *    في الطباعة **تشديدٌ** لا متن: يُستعمل لكلمةٍ أو جملة، وصفحةٌ
     *    كاملةٌ منه تُتعِب العين وتقول «عرضٌ» لا «قراءة». وصفحةُ
     *    الجُمَل هي ما تقرؤه دقائقَ متّصلة، فتستحقّ وجهًا رومانيًّا.
     *
     *    و`noto` (Noto Serif) أوّلُ الخطوط في القائمة، سيريليٌّ
     *    مُثبَت، ووجهُ كتابٍ محايدٌ بلا مبالغة (بند ١٣).
     *
     * ⚠️ **ولا خيارَ حُذف**: العشرةُ كلُّها في منتقي «Aa»، ومنها
     *    `philosopher`. وجلسةٌ حفظت `fontDocId` تفتح بما اخترتَه لها
     *    — هذا الافتراضُ لمن لم يختر بعد.
     *
     * ⚠️ **والوراثةُ من `fontId` باقيةٌ — وحذفُها كان خطئي.**
     *
     *    كتبتُ أوّلًا `session.fontDocId || 'noto'` بحجّة أنّ الصفحتين
     *    غرضان مختلفان. فسقط حارسٌ قائمٌ: «الجلساتُ القديمة تفتح كما
     *    تركتَها». وكان محقًّا — الجلسةُ التي اخترتَ لها خطًّا قبل أن
     *    ينقسم الحقلان كانت صفحتاها به، فإسقاطُ الوراثة **يبدّل شاشةً
     *    تحت يدك**. وذلك وعدٌ بحفظ ما تركتَه، لا تفضيلُ تنسيق.
     *
     *    والافتراضُ الجديدُ لا يحتاج كسرَ الوعد أصلًا: `fontId` لا
     *    يُكتَب إلّا حين تختار خطَّ المسرح بيدك، فالجلسةُ الجديدةُ
     *    تصل إلى `'noto'` من الطرف الثالث من السلسلة. **المقايضةُ
     *    التي تظنّها لازمةً كثيرًا ما تكون رقمًا لم تقرأه.**
     */
    fontDoc: session.fontDocId || session.fontId || 'noto',
  ...
  /* زرُّ الصوت فوق الميكروفون يقول الصوتَ الحاليّ منذ الرسم الأوّل. */
  paintVoiceShortcut();
        ...
        <div class="sh-book">
          ...
          <div class="sh-pages" data-pages>
            ...
            <div class="sh-page sh-left">
              ...
              -->
              <!--
                ⚠️ **وهامشُ الأرقام يتّسع للرقم الأطول** (WS-BG · بند ٣).
                   الهامشُ ثابتٌ بالبكسل، ورقمُ ثلاثِ خاناتٍ أعرضُ منه:
                   قِستُ «١٠٠» بخطّ الأرقام فكان ٢١٫٨px وهامشُنا ١٧.
                   فالجلسةُ الطويلةُ كان رقمُها يركب أوّلَ الحروف —
                   وهو عطبٌ قائمٌ قبل هذه التمريرة لا جديد.

                   والحلُّ خانةٌ واحدةٌ في الوسم لا حسابٌ في CSS: الرسمُ
                   يعرف عددَ الجُمَل، وCSS لا تعرفه.
              -->
              <div class="sh-lines" data-lines
                ${segments.length >= 100 ? 'data-wide-n' : ''}>
            ...
            <div class="sh-page sh-right">
              ...
              <div class="sh-stage-top">
                ...
                <!--
                  ══════════ عنقودُ الجملة في رأس المسرح (WS-TOOLS-LAYOUT · بندا ٦ و٧) ══════════

                  ⚠️ **كانا داخل البطل، والبطلُ صار يُمرَّر** (WS-HEROSCROLL).
                     شارةُ «ارجع للأصل» وأزرارُ الحفظ/النسخ/المفضّلة كانت في
                     sh-hero-top فوق النصّ الممرَّر مباشرةً — فكلُّ سحبةٍ رأسيّةٍ
                     تبدأ عليها هي سحبةٌ لا تُمرّر، وكلُّ ضبطٍ لارتفاع البطل
                     يحرّكها. ورأسُ المسرح **لا يُمرَّر أبدًا**: العدّادُ وشريطُ
                     التكرار فيه، فموضعُهما ثابتٌ بحكم بنيته لا بقاعدةٍ تُضاف.

                  ⚠️ **ولا زرَّ تغيّر ولا مُعالِجَ نُسِخ**: نفسُ العناصر بنفس
                     السمات (save-item · copy-item · panel=difficult · scratch-clear)
                     تحت أبٍ آخر. والتفويضُ على app-main فلا يعرف الأبَ أصلًا.

                  ⚠️ **والشارةُ تبقى الهدفَ الوحيدَ لـ scratch-clear** — راجع
                     التعليقَ عند دالّة طلائها: هي زرُّ الخروج نفسُه، لا نسخةٌ ثانية.
                -->
                ...
                <div class="sh-hero-top">
                  <!--
                    ⚠️ **شريطُ المقطع الجزئيّ** (WS-A، بند ٣٣). يقول ما
                       ...
                       الأدوات في اللوح، وهذا **إخبارٌ لا تحكّم**.

                    ⚠️ **وهو وحدَه بقي هنا** (WS-TOOLS-LAYOUT · بندا ٦ و٧):
                       الشارةُ وأزرارُ الجملة صعدت إلى رأس المسرح الثابت،
                       وهذا خبرٌ عن **النصّ الذي تحته** فيقرأ معه ويبقى فوقه.
                  -->
              ...
              <!--
                ══════════ بابٌ واحدٌ للإعدادات (WS-SCLEAN · بندا ٤ و٢٨) ══════════
                     ثم صار **لسانَ حافّةٍ** (WS-POLISH · بنود ٥-٨)
                ...
                ⚠️ **وحتى البابُ الواحدُ كان يحجز صفًّا — فصار لسانًا
                   على الحافّة** (WS-POLISH · بند ٥). قِيس: صفُّ
                   الرقاقات 376×32 عند الحدّ الأسفل، بهامشٍ ٨ = **٤٠px
                   من ارتفاع المسرح** لزرٍّ واحدٍ لا يُلمَس إلّا حين
                   تضبط. والمسرحُ للغة.

                   فصار الزرُّ **مطلقَ الموضع فوق الحافّة**: لا يحجز
                   عرضًا ولا ارتفاعًا في مسار المحتوى — والقياسُ يُثبته
                   (ارتفاعُ نطاقِ الكلمات لم يتغيّر بوجوده).

                ⚠️ **ومكانُه الزاويةُ السفلى اليسرى — مقيسًا لا مُختارًا**
                   (بند ٨). فُحِصت الحافّتان في الثلث الأسفل:
                     · اليمنى: سكّةُ الأدوات (٤٨px مطلقة) ومقبضُها
                       ٤٤×٤٤ في أسفلها — فوضعُ الترسِ هناك يصنع
                       **سكّةً ثانية**، وهو المنهيُّ عنه (بند ٦).
                     · اليسرى في منتصف الثلث الأسفل: نطاقُ الكلمات
                       يمتدّ إليها (elementFromPoint يقول sh-chip) —
                       فالترسُ يعلو كلمةً تُلمَس.
                   والحرُّ فعلًا: **يسارُ شريط النقل** — أزرارُه
                   متوسّطةٌ تبدأ بعد ٦٠px من حدّ المسرح، فتبقى الزاويةُ
                   خاليةً وبعيدةً عن الترتيب: تشغيلٌ في الوسط، وسكّةٌ
                   يمينًا، وضبطٌ يسارًا.
              -->
              <button class="sh-cc-tab" data-sh="drawer" data-dial="speed"
                aria-label="اضبط التدريب"><span aria-hidden="true">⚙</span></button>

              <!-- الحجاب: ضغطةٌ خارج اللوح تغلقه. -->
              <div class="sh-scrim" data-sh="rail-close"></div>

              <!--
                ══════════ عنقودُ الحافّة السفليّة (WS-TOOLS-LAYOUT · بند ٤) ══════════

                ⚠️ **السكّةُ الطوليّةُ رُفعت من المسرح** — لا عمودَ ولا خلفيّةَ
                   ولا حصّةَ عرضٍ محجوزة. وما كان فيها لم يُحذَف منه فعلٌ
                   واحد: «سجّل غلطة» صار في مركز التدريب، و«تعلّم من الجملة»
                   و«Аа» هنا، وأدواتُ الكلمة في رأس اللوح نفسِه.

                ⚠️ **وهذه أبوابٌ لا أفعالٌ ثانية**: كلُّ زرٍّ هنا يُصدِر
                   data-sh=tool بنفس data-v الذي يُصدره السجلّ — فالمُعالِجُ
                   واحدٌ (pickTool) والحالةُ واحدة. ولا نسخةَ منطقٍ ثانية.

                ⚠️ **وهي طبقةٌ عائمةٌ لا عمودُ تخطيط**: position: absolute
                   فلا تقتطع من عرض القراءة ولا تُزيح مركزَ زرّ التشغيل.
              -->
              <div class="sh-edge" data-edge>
                <!--
                  ⚠️ **وتقدّمُ المسودّة يُكتَب هنا** (بند ٥): كان سطرًا ثانيًا
                     داخل زرّ السكّة العريض («٣/٨»)، ولا يسعه زرُّ اللوح ٣٦px.
                     وهو خبرٌ يُقرأ في نظرة — فصعد إلى الزرّ الذي صار بابَ
                     المسودّة. والقيمةُ من سجلّ الأدوات نفسِه لا من حسابٍ ثانٍ.
                     (⚠️ ولا علامةَ اقتباسٍ خلفيّةً هنا — تُنهي وسمَ html ولو
                      كانت داخل تعليق. وهي الثانيةَ عشرةَ التي أوقعتني.)
                -->
                <button class="sh-edge-btn" data-sh="tool" data-v="learn"
                  aria-label="تعلّم من الجملة"><span aria-hidden="true">✦</span
                  ><i class="sh-edge-val" data-edge-val hidden></i></button>
                <!--
                  ⚠️ **معاينةُ الخطّ بابٌ ثانٍ إلى نفس الإعداد لا إعدادٌ ثانٍ**
                     (WS-MIC · ٢-٤): تُصدِر فعلَ اختيار الخطّ نفسَه الذي
                     يُصدره المركز، فتكتب حقلَ خطّ المسرح وتحفظه. وتبقى
                     اللوحةُ مفتوحةً بعد الاختيار لتقارن والجملةُ أمامك.
                -->
                <button class="sh-qfont sh-edge-btn" data-sh="qfont"
              ...
              <aside class="sh-panel" data-panel-host>
                ...
                <!--
                  ══════════ أدواتُ السياق في رأس اللوح (بند ٥) ══════════

                  ⚠️ **الصندوقُ هو الصندوقُ نفسُه بسمته نفسِها** — دالّةُ رسم السكّة
                     تناديه بالسمة لا بموضعه، والنقرُ مفوَّضٌ على app-main.
                     فلم يتغيّر مُعالِجٌ ولا سجلُّ أدواتٍ ولا حالة: تغيّر أبٌ.

                  ⚠️ **وخارج جسم اللوح عمدًا**: الجسمُ يُعاد بناؤه
                     بإسنادِ innerHTML مع كلّ رسم، فلو كان الصفُّ داخلَه لهُدم معه.

                  ⚠️ **ولا يُفتَح اللوحُ بها بل بها يُختار**: الضغطةُ المطوّلةُ
                     على كلمةٍ تفتح اللوحَ أصلًا (قِيس في الطور الأوّل: يصير
                     صنفُ الفتح صحيحًا)، فتجد أدواتَها أمامك في رأسه.
                -->
    ...
    <aside class="sh-drawer" data-drawer hidden aria-label="اضبط التدريب">
      ...
      <div class="sh-drawer-body">
        <section class="sh-cc-sec">
          ...
          <!--
            ══════════ عددُ التكرار: منزلقٌ كأخواته (WS-POLISH · بنود ١-٤) ══════════

            ⚠️ **بلاغُك**: «مش عايز أكتب عدد التكرار بإيدي» — وكان الصفُّ
               ثلاثَ طبقاتٍ لإعدادٍ واحد: ثمانيةُ أزرارٍ جاهزة، ثم خانةُ
               رقمٍ كبيرة، ثم لا منزلق. فكان أضخمَ صفٍّ في اللوحة
               (١٥٣px) وأقلَّها اتّساقًا: السرعةُ والفاصلُ والصوتُ
               منزلقاتٌ، وهو وحدَه أزرارٌ وكتابة.

            ⚠️ **والمدى مقيسٌ لا مُختار**: سجلُّ الضبط (TUNERS) يقول
               min: 1 · max: 99 · step: 1 — وهو المرجعُ الذي يقصّ
               القيمةَ ويحفظها. فلا مدى جديدًا هنا، ولا خطوةً
               كسريّةً تُظهر رقمًا وتحفظ غيرَه: كلُّ موضعٍ للمنزلق
               عددٌ صحيحٌ موجود، والمكتوبُ فوقه هو المحفوظُ نفسُه.

            ⚠️ **والأزرارُ الثمانيةُ أُزيلت** (بند ٣): بعد المنزلق لا
               تُسرّع شيئًا — الوصولُ إلى ٢١ سحبةٌ واحدةٌ بالإبهام —
               وكانت تحجز صفّين وتُنافس المنزلقَ على الانتباه.
          -->
          ${raw(ccRow({
            key: 'repeat', label: 'عدد التكرار', value: `×${reps}`,
            body: html`<input type="range" class="sh-range" data-tune-range="repeat"
              min="1" max="99" step="1" value="${reps}"
              aria-label="عدد التكرار" aria-valuetext="${reps} مرّة" />`,
          }))}
        ...
        <!--
          ══════════ سجّلُ الغلط — بابُه هنا بعد رفع السكّة (WS-TOOLS-LAYOUT · بند ٣) ══════════

          ⚠️ **كان في السكّة الطوليّة، وقد رُفعت.** وما بقي من السكّة صار
             صفًّا **سياقيًّا** في رأس اللوح: لا يُرى إلّا وفي يدك كلمةٌ أو
             مقطع. وسجّلُ الغلط لا شرطَ له — يعمل على الكلمة الممسوكة أو
             على الجملة كلِّها — فمكانُه المركزُ الذي يُفتَح متى شئت.

          ⚠️ **ولا فعلَ ثانٍ ولا مُعالِجَ ثانٍ**: data-sh=tool وdata-v=mistake
             هما نفسُ ما كانت تُصدره السكّة، فيدخلان pickTool ويصلان إلى
             openErrorCapture بالحرف. وبصريًّا هذا **فعلٌ** لا إعداد، فلا
             يدخل صفوفَ ccRow ولا يحمل قيمةً تُقرأ في الملخّص.
        -->
        <section class="sh-cc-sec">
          <h3 class="sh-cc-h">أثناء التدريب</h3>
          <div class="sh-cc-row">
            <div class="sh-cc-head"><span class="sh-cc-lbl">غلطتَ في حاجة؟</span></div>
            <div class="sh-cc-chips">
              <button type="button" data-sh="tool" data-v="mistake">⚠ سجّل غلطة</button>
            </div>
          </div>
        </section>
...
function handleEvent(event) {
  /*
   * ⚠️ **المحرّكُ لا يُصدر صوتًا بلا مالك** (Voice Center V1.0C · الخطوة ٣).
   *    عنصرا المحرّك — `humanEl` للتسجيل البشريّ و`audioEl` في مُكيِّف
   *    النطق — لا يبدآن إلّا بعد `start` أو `resume`. وكان زرُّ ▶ وحدَه
   *    يطالب بالناقل؛ أمّا ▶ الشريطِ العائم، والبدءُ بعد لصق نصٍّ،
   *    والاستئنافُ بعد نافذة التعبير — فكانت تُشغّل المحرّكَ والناقلُ
   *    بلا مالكٍ أو بمالكٍ آخر، فلا يُسكِته تسجيلٌ يبدأ ولا يُسكِت هو
   *    ما يعمل. فالمطالبةُ هنا، **قبل** فرع المتروك، لتشمل الأبوابَ كلَّها.
   */
  if ((event.type === 'start' || event.type === 'resume') && !ownsAudio('session')) {
...
function syncSegment() {
  ...
  if (textEl) {
    /*
     * ══════════ الجملةُ تُكتَب لقطةً واحدة (WS-DRAFT-SYNC · العطب ١) ══════════
     *
     * ⚠️ **بلاغُك**: «النصُّ الأبيضُ جملةٌ والنبرُ الذهبيُّ جملةٌ أخرى».
     *
     * ⚠️ **والوسمُ لم يكن يخلط قطُّ** — قِيس: عشرون تنقّلةً سريعة، والنصُّ
     *    وعددُ نبراته متطابقان في كلّ مرّة (أ=٢ نبرة، ب=١١)، ولا نبرةً
     *    شاردةً واحدة، حتّى بأخذ اللقطة **قبل** الاستقرار. فالسطران
     *    تحت هذا التعليق يكتبان النصَّ والنبرَ في إسنادٍ واحد.
     *
     * ⚠️ **والذي كان يكذب هو الموضع**: البطلُ صار مُمرِّرًا بنفسه
     *    (WS-HEROSCROLL)، و`innerHTML` **لا يصفّر التمرير**. فالمتصفّحُ
     *    يُبقي إزاحةَ الجملة السابقة ويقصّها على ارتفاع الجديدة. قِيس:
     *
     *        الجملةُ الأولى مُمرَّرةٌ إلى آخرها      scrollTop = ٨٢
     *        ثمّ «التالي»                          scrollTop = ٤٦
     *        فتبدأ الجملةُ الجديدةُ من **وسطها**، وسطرُها الأوّل مخفيّ
     *
     *    فتُقرأ شظيّةٌ من الجملة الجديدة بنبراتها فوق ما تتذكّره عينُك
     *    من القديمة — وهو «الشاشة اتلخبطت» بحرفه.
     *
     * ⚠️ **والتصفيرُ جزءٌ من نفس الإسناد** لا خطوةٌ تالية: لقطةٌ واحدة
     *    تحمل النصَّ والنبرَ والموضع، فلا يُرى إطارٌ واحدٌ بأحدها دون
     *    الآخر. ولا يُصفَّر إلّا حين تتبدّل الهُويّة فعلًا — فإعادةُ
     *    رسمٍ لنفس الجملة (وصولُ نبرٍ من الشبكة مثلًا) لا تسرق منك
     *    موضعَ قراءتك.
     */
    const stamp = `${segment.id || ''}|${segment.draftId || ''}|${segment.targetId || ''}`;
    const fresh = textEl.dataset.seg !== stamp;
   ...
   */
  /*
   * ⚠️ **والدرجاتُ رُفعت في WS-ST** (بند ٤): كانت `.62/.78/1`، فكانت
   *    جملةٌ من أربعَ عشرةَ كلمةً تُقرأ بـ**٢٣٫٤px** — أصغرَ من
   *    المدى المطلوب (٣٢–٤٤) وأقربَ إلى نصّ قائمةٍ منها إلى جملةٍ
   *    على مسرح. والانكماشُ يبقى لأنّه يمنع الفيض، لكنّه صار
   *    **تنازلًا طفيفًا** لا انهيارًا.
   *
   *    والأثرُ على الافتراض (٣٠px) بالحساب:
   *
   *        الطول            قبل            بعد
   *        ≤٨٠ حرفًا       ٣٠ × ١  = ٣٠    ٣٠ × ١  = ٣٠
   *        ٨١–١٥٠         ٣٠ × .78 = ٢٣٫٤  ٣٠ × .9 = **٢٧**
   *        >١٥٠           ٣٠ × .62 = ١٨٫٦  ٣٠ × .78 = **٢٣٫٤**
   *
   *    فالجملةُ الطويلةُ جدًّا صارت تُقرأ بما كانت المتوسّطةُ تُقرأ به.
   *    والانكماشُ باقٍ لأنّه يمنع الفيض — لكنّه صار تنازلًا طفيفًا.
   */
  const chars = (segment.sourceTextSnapshot || '').length;
  const app = document.querySelector('.shadow-app');
  if (app) app.style.setProperty('--sh-len', chars > 150 ? '.78' : chars > 80 ? '.9' : '1');
   ...
   *
   *    والحارسُ يمنع أثرًا متأخّرًا من الوصول بعد أن تكون قد انتقلتَ
   *    لجملةٍ أخرى.
   *
   * ⚠️ **وكان الحارسُ رقمًا، والرقمُ يُعاد استعمالُه** (WS-DRAFT-SYNC · العطب ١-ب).
   *
   *    كان الشرطُ `player.state.index === at` — أي فهرسًا عدديًّا. وهو
   *    صادقٌ في المصدر الثابت، وكاذبٌ في **المسودّة**: الدخولُ إلى
   *    أهدافِ مسودّةٍ يستدعي `dropExternalSource()` ثمّ يُلحِق المقاطعَ
   *    الجديدة، فتبدأ من **نفس الفهرس** الذي تركه الطقمُ السابق. فطلبُ
   *    نبرٍ بدأ للهدف أ يمكن أن يجد `index` كما تركه ويمرّ — وقد صار
   *    المعروضُ الهدفَ ب.
   *
   *    والأثرُ ليس خلطًا في الوسم (`syncSegment` تقرأ الحالةَ الجارية
   *    فترسم ب كاملةً)، بل أنّ ما جُلب لأجل أ يدخل قاموسَ النبر ثمّ
   *    يُطبَّق على كلمات ب — ومتجانساتُ الروسيّة كثيرة (за́мок/замо́к).
   *    فتظهر على ب نبرةٌ مصدرُها أ. وهو نصفُ بلاغك الآخر.
   *
   *    فصار الحارسُ على **الهُويّة** التي تختمها اللقطةُ نفسُها: معرّفُ
   *    المقطع ومسودّته وهدفه. ورقمٌ مُعادُ استعمالُه لا يخدعها.
   */
  if (ctx.stress) {
    const at = `${segment.id || ''}|${segment.draftId || ''}|${segment.targetId || ''}`;
    void fetchSentenceStress(segment.sourceTextSnapshot || '')
      .then((changed) => {
        if (!changed) return;
        const now = ctx?.segments?.[player?.state?.index];
        if (!now) return;
        const nowStamp = `${now.id || ''}|${now.draftId || ''}|${now.targetId || ''}`;
        if (nowStamp === at) syncSegment();
      })
      .catch(() => {});
  }
   ...
   */
  /*
   * ⚠️ **والتظليلُ ينتقل قبل الرسم لا بعده** (WS-DSCTME · العطب ١).
   *    `renderWells` تقرأ القاعدةَ كلَّها قبل أن تكتب، فتظليلُ الهدف
   *    الجاري كان يتأخّر عن نصّه بقدرِ تلك القراءة. والهُويّةُ في يدنا
   *    هنا (`segment.targetId`)، فتُكتَب الآن؛ ثمّ يأتي الرسمُ الكاملُ
   *    فيبني ما تبدّل من بطاقات.
   */
  if (well === 'draft') {
    if (paintWellTarget(segment.targetId || '') && !wellPinned) revealWellTarget();
    renderWells().catch(() => {});
  }
  /*
...
const TOOLS = [
  ...
  /*
   * ⚠️ **وسجّلُ الغلط غادر هذه القائمة إلى مركز التدريب**
   *    (WS-TOOLS-LAYOUT · بند ٣). وسببُ مغادرته أنّ هذه القائمةَ صارت
   *    **سياقيّةً** بعد رفع السكّة: أزرارُها تُرسَم في رأس اللوح الذي
   *    لا يُفتَح إلّا على كلمةٍ أو مقطع. وسجّلُ الغلط بلا شرطٍ (لا when
   *    له) — فلو بقي هنا لظهر فعلٌ عامٌّ داخل صفٍّ خاصٍّ بما في يدك.
   *
   * ⚠️ **ولم يُنسَخ منطقُه**: زرُّ الدرج يُصدِر data-sh=tool بنفس data-v،
   *    فيدخل pickTool ويصل إلى openErrorCapture كما كان بالحرف.
   *    وقيمةُ rail.tool الابتدائيّةُ تحوّلت إلى learn لأنّ mistake
   *    لم يعد في السجلّ — ومُنتقٍ يشير إلى غائبٍ هو الموتُ الصامت نفسُه.
   */
  /*
...
const rail = {
  /*
   * ⚠️ **والافتراضُ أداةٌ في السجلّ — وإلّا فلوحةٌ لا بابَ لها (WS-SCLEAN).**
   *    كان `display` فخرجت إلى مركز التدريب، فصار الافتراضُ يشير إلى
   *    غائب. ثم صار `mistake` — وقد خرج هو الآخرُ إلى المركز
   *    (WS-TOOLS-LAYOUT · بند ٣)، فوقع العطبُ نفسُه مرّتين باسمين.
   *
   *    و`renderRail` تُصحّح ذلك عند أوّل رسم، لكنّ الافتراضَ الخاطئ
   *    يعني لوحةً غيرَ متوقّعةٍ في أوّل لمسةٍ قبل التصحيح. و«تعلّم من
   *    الجملة» أداةُ مسرحٍ بلا شرط — فلا تغيب عن السجلّ أبدًا.
   */
  open: false, tool: 'learn', word: -1,
};
...
function renderRail() {
  ...
  /*
   * ⚠️ **وزرُّ المسودّة في عنقود الحافّة يُضاء من نفس القيمة**
   *    (WS-TOOLS-LAYOUT · بند ٤): لا حسابَ ثانيًا ولا نداءَ ثانيًا إلى
   *    `learning` — تُقرأ القيمةُ من سجلّ الأدوات نفسِه في نفس الرسم،
   *    فلا يمكن أن يقول الزرّان رقمين مختلفين.
   *
   * ⚠️ **ويُسمّى صنفُ «على» ليقول أنّ اللوحَ مفتوحٌ عليه** — وإلّا بقي
   *    بابُ المسودّة بلا أثرٍ يقول إنك داخله بعد أن زال إضاءةُ السكّة.
   */
  const edgeBtn = $('.sh-edge [data-v="learn"]');
  const edgeVal = $('[data-edge-val]');
  if (edgeVal) {
    const v = TOOLS.find((t) => t.id === 'learn')?.value?.() || '';
    edgeVal.textContent = v;
    edgeVal.hidden = !v;
  }
  if (edgeBtn) edgeBtn.classList.toggle('on', rail.open && rail.tool === 'learn');

  /*
   * ⚠️ **ومقبضُ السكّة زال مع سكّته** (WS-TOOLS-LAYOUT): كان هذا السطرُ
   *    يقلب سهمَه. ولا يُترَك مُنتقٍ ميّتٌ في الشيفرة — فذاك بعينه ما
   *    أعمى خطَّ النصّ الكامل تمريرةً كاملة: مُنتقٍ لا يطابق شيئًا يمرّ
   *    صامتًا ولا يُخطئ بصوتٍ عالٍ. واللوحُ يُغلَق بالحجاب أو بزرّ ✕.
   */
  if (!rail.open) return;
  ...
  if (body) {
    /*
     * ══════════ اللوحُ لا يعود إلى أوّله مع كلّ نقلة (WS-DRAFT-SYNC · العطب ٢) ══════════
     *
     * ⚠️ **بلاغُك**: «اللوحُ بيرجع لأوّله كلّ ما أنتقل من هدفٍ لهدف».
     *
     * ⚠️ **وقِيس فكان كما قلتَ بحرفه** (لوحٌ مفتوحٌ فعلًا، ١٦ هدفًا):
     *
     *        scrollTop قبل النقلة   ٠
     *        scrollTop بعدها        ٠        ← وقد بُني اللوحُ من جديد
     *        الهدفُ الجاري          ١١٤٤..١٢٣٣
     *        نافذةُ اللوح            ١٤٢..٥٢٩   ← الهدفُ تحت الطيّة بـ٦١٥px
     *
     *    والسببُ هنا في هذا السطر: `innerHTML` يستبدل محتوى **حاوية
     *    التمرير نفسِها**، فيسقط موضعُ التمرير إلى الصفر حتمًا. ثمّ لا
     *    يُكشَف الهدفُ الجاري أبدًا، فتبحث عنه بإصبعك في كلّ مرّة.
     *
     * ⚠️ **والموضعُ يُلتقَط قبل الهدم ويُعاد بعده** — والكشفُ يقع في
     *    `renderLearn` بعد أن يكتمل المحتوى، لا هنا: هذه قشرةٌ والمحتوى
     *    يأتي بعدها. راجع الشرحَ هناك.
     *
     * ⚠️ **ولا يُقاس شيءٌ من هذا ولوحُه مغلق**: قِستُ أوّلًا فبدا اللوحُ
     *    محتفظًا بموضعه (٦٥٦ ← ٦٥٦) — وكان مغلقًا (`opacity: 0`) فلا
     *    يُعاد بناؤه أصلًا. المِرقابُ كان يقيس لوحًا لا يراه أحد.
     */
    panelKeep = body.scrollTop;
...
async function pickTool(id) {
  ...
  /*
   * ⚠️ **ويُغلَق الدرجُ معه بعد أن صار بابُه هناك** (WS-TOOLS-LAYOUT · بند ٣).
   *    نافذةُ تسجيل الغلط `showModal` فوق كلّ شيء، والدرجُ تحتها يبقى
   *    مفتوحًا فتجده أمامك حين تُغلقها — وأنت تظنّ أنك غادرته. وإغلاقُه
   *    غيرُ ضارٍّ إن لم يكن مفتوحًا أصلًا (السكّةُ تُغلَق هكذا منذ البداية).
   */
  if (id === 'mistake') {
    rail.open = false;
    renderRail();
    toggleDrawer(false);
    return openErrorCapture();
  }
  if (id === 'myvoice') {
    rail.open = false;
    renderRail();
...
const MODES = [
  ...
  /*
   * ══════════ ⚠️ «متّصل» رُفع من المفتاح — وهذا قبرُه وسببُه ══════════
   *
   * كان هنا وضعٌ رابعٌ اسمُه «متّصل»، وكتبتُ فوقه يومَ أضفتُه (WS-SCLEAN)
   * أنّه «يمرّ على المقاطع تباعًا بلا تكرارٍ لكلٍّ»، وأنّه **لا يُحَلُّ
   * بحذف الخيار** لأنّه غيرُ «مستمرّ» في مركز التدريب.
   *
   * ⚠️ **وكان ذلك خطأً مكتوبًا بثقة.** جُرِّد المحرّكُ قبل أن يُرفَع سطر،
   *    بشاهدٍ موجَبٍ وآخرَ سالب (١٫٨ ثانيةٍ لكلّ وضع، مُنطِقٌ محقون):
   *
   *      جملة   ← ١، ١، ٢   الفهرس ١   seek#1
   *      متّصل   ← ١، ١، ٢   الفهرس ١   seek#1     ← **الحرفُ نفسُه**
   *      كلمة   ← يختلف      الفهرس ٠   word-select
   *
   *    فالشاهدُ الموجَبُ يُثبت أنّ المسبارَ يرى الفرقَ حين يوجد، ثمّ يشهد
   *    بأن لا فرقَ هنا. وقراءةُ المصدر تؤكّده: `PRACTICE_MODE.CONTINUOUS`
   *    **لا تُقرأ في المحرّك ولا مرّةً واحدة**.
   *
   *    والذي «يمشي على الجمل بلا تكرار» حقلٌ آخرُ اسمُه `repeatMode`،
   *    وبابُه مركزُ التدريب («بعد ما يخلّص العدد ← مستمرّ»). فالقدرةُ
   *    التي وَعَد بها الاسمُ قائمةٌ ولم تُمَسّ — لكنّها لم تكن تحته قطّ.
   *
   * ⚠️ **فما رُفع زرٌّ يعمل، بل رابعُ خيارٍ يقول ما لا يفعل** — وهو
   *    بالضبط العيبُ الذي جاء ذاك التعليقُ نفسُه ليمنعه: «شاشةٌ تقول
   *    غيرَ حالها أسوأُ من خيارٍ ناقص».
   *
   * ⚠️ **والجلساتُ المحفوظةُ عليه تُفتَح وتعمل**: `normalizePracticeMode`
   *    تطوي القيمةَ المهجورةَ إلى «جملة» عند البناء وعند التحديث معًا،
   *    و`renderModes` تكتبها مرّةً واحدةً في السجلّ حتّى لا تبقى الحالةُ
   *    المخزَّنةُ تقول شيئًا والشاشةُ تُضيء غيرَه.
   */
];
...
function renderModes() {
  ...
  if (!host || !ctx) return;
  /*
   * ⚠️ **جلسةٌ محفوظةٌ على «متّصل» تُكتَب مرّةً واحدةً ثمّ تُنسى**
   *    (WS-FINAL-VISUAL · بند ٢). المحرّكُ يطبّعها عند البناء، لكنّ
   *    **السجلَّ** يبقى يقول `continuous` إلى أن يبدّل المستخدمُ وضعًا.
   *    فلو قرأها غدًا قارئٌ آخرُ لوجد قيمةً لا مفتاحَ لها.
   *
   *    والكتابةُ مشروطةٌ بالفرق، فلا تُكتَب في كلّ رسم — و`renderModes`
   *    تُنادى من ثمانية مواضع.
   */
  const saved = ctx.session?.practiceMode;
  if (saved === 'continuous') {
    ctx.session.practiceMode = PRACTICE_MODE.SENTENCE;
    void saveSessionSettings(ctx.session.id, { practiceMode: PRACTICE_MODE.SENTENCE })
      .catch(() => {});
  }
...
/*
 * موضعُ تمرير اللوح يعبر إعادةَ البناء (WS-DRAFT-SYNC · العطب ٢)، وآخرُ
 * هدفٍ كُشِف يمنع اللوحَ من مصارعة إصبعك: لا يتحرّك إلّا حين يتبدّل الهدفُ
 * فعلًا، فإن كنتَ تقرأ وسطَ اللوح ولم ينتقل التدريبُ بقي حيث تركتَه.
 */
let panelKeep = 0;
...
async function renderLearn() {
  ...
  /*
   * ══════════ اللوحُ يتبع الهدفَ الجاري — ولا يصارع إصبعك ══════════
   *
   * ⚠️ **الموضعُ أوّلًا**: المحتوى اكتمل الآن، فيُعاد موضعُ التمرير الذي
   *    التُقط قبل الهدم في `renderRail`. بدونه يبدأ اللوحُ من الصفر مع
   *    كلّ نقلةٍ — وهو بلاغُك بحرفه.
   *
   * ⚠️ **ثمّ الكشفُ بالهُويّة الثابتة لا بالموضع**: `activeTargetId` هو
   *    معرّفُ هدفِ المسودّة نفسِه (`DT_…`) — موجودٌ في التطبيق منذ WS-DV3،
   *    فلا يُخترَع فهرسٌ عدديّ.
   *
   * ⚠️ **و`nearest` لا `center`**: يتحرّك بأقلّ ما يكشف. فإن كان الهدفُ
   *    داخل النافذة أصلًا لم يتحرّك بكسلًا — وهو شرطُك الرابع.
   *
   * ⚠️ **ولا يُكشَف إلّا حين يتبدّل الهدف**: إعادةُ رسمٍ بريئةٌ (وصولُ
   *    نبرٍ من الشبكة، أو تبدّلُ حالةِ هدفٍ) لا تخطف موضعَ قراءتك. وهو
   *    شرطُك الثامن: لا يقفز واللوحُ ساكنٌ وأنت تقرأ.
   */
...
function paintTransport() {
  ...
  /*
   * ⚠️ **وتُمسَك الصفحةُ حيّةً ما دامت تنطق (WS47، وأُصلحت آليّتُها في
   *    WS51).** بلاغُك: «الصوت مشتغلش لما بقفل التابلت». والسببُ أن
   *    المتصفّح يُجمّد الصفحةَ في الخلفية فتموت مؤقّتاتُ الجلسة؛
   *    والمانعُ الحقيقيّ `AudioContext` شغّالٌ يُفتَح **من هذه الضغطة
   *    بالذات** — والشرحُ الكامل في `background-audio.js`.
   *
   * ⚠️ **فلا تُدخِل `await` بين النقرة وهذا السطر.** الطريقُ اليوم
   *    متزامنٌ كلُّه: النقرة ← `togglePlay` ← هنا ← فتحُ السياق. وأوّلُ
   *    `await` يُدرِجه أحدٌ في الطريق يُبطل الأثرَ كلَّه بلا أن يكسر
   *    اختبارًا واحدًا على سطح المكتب — ولا يظهر إلّا حين تُقفل اللوح.
   *
...
function paintLines() {
   ...
   *    السطرُ فوقَه يستبدل `innerHTML` كلَّه — فتضيع مع كلّ نقلةِ
   *    جملةٍ وكلّ تبديلِ وضعِ قراءة. وهو الدرسُ نفسُه الذي دفع
   *    الرقائقَ إلى متغيّرٍ على الجذر بدل نمطٍ سطريّ.
   *
 ...
 */
/*
 * ⚠️ **و«بدون تنسيق» عرضٌ بلا قاعدةٍ واحدة — وهذا تعريفُه لا اختصارُه.**
 *    العروضُ الثلاثةُ مطليّةٌ بـ`[data-ft-view="…"]`، فعرضٌ لا تُكتَب
 *    له قاعدةٌ **هو** الفقرةُ المتّصلةُ كما كانت قبل أن تُضاف العروض.
 *    ولو رُسم بقواعدَ «تُلغي» الثلاثةَ لصار تقليدًا للأصل يفترق عنه
 *    يومَ يتغيّر أحدُهما — والأصلُ نفسُه أصدقُ من محاكاته.
 *
 * ⚠️ **وهو الافتراض** (طلبُك): الفتحةُ الأولى ترى ما كانت تراه.
 */
const FT_VIEWS = Object.freeze([
  { id: 'plain', glyph: '¶', label: 'بدون تنسيق' },
...
/**
 * تذكرةُ رسمِ اللوح — **آخرُ نداءٍ وحدَه يكتب** (WS-DSCTME · العطب ١).
 *
 * ⚠️ `renderWells` تنتظر قراءاتِ **كلّ** المنابع من القاعدة قبل أن
 *    تكتب حرفًا. ونداءان متداخلان (نقلتا هدفٍ متلاحقتان) ينتهيان
 *    بالترتيب الذي تُنهيه به القاعدةُ لا بالترتيب الذي بدآ به —
 *    فيكتب الأقدمُ فوق الأحدث ويبقى اللوحُ على هدفٍ غادرتَه.
 */
let wellDrawTicket = 0;
...
/**
 * ينقل صنفَ «الهدف الجاري» على بطاقات لوح المسودّة **الآن** — بلا
 * قراءةِ قاعدةٍ ولا انتظارِ رسم (WS-DSCTME · العطب ١).
 *
 * ⚠️ **وبلاغُك: «التظليل يبدو تابعًا لجملةٍ أخرى، أو يسبق المعروض».**
 *    والسببُ مقيسٌ: البطلُ والرقائقُ يُكتَبان في `syncSegment` **تزامنيًّا**
 *    (٤–٥ms)، وتظليلُ اللوح لا يتحرّك إلّا بعد أن تعود `renderWells`
 *    من قراءة **كلّ** منبعٍ من IndexedDB. فبينهما إطارٌ على هذه الآلة،
 *    و**٤٠٠ms** حين أُبطئت القراءةُ إلى وتيرةِ جهازٍ حقيقيّ — وهي النافذةُ
 *    التي ترى فيها نصَّ هدفٍ وتظليلَ آخر.
 *
 * ⚠️ **والهُويّةُ في اليد أصلًا**: المقطعُ يحمل `targetId`، والبطاقةُ
 *    تحمل `data-dw-target` منذ WS-DI. فلا قراءةَ ولا حسابَ — تبديلُ
 *    صنفٍ على شجرةٍ مرسومة.
 *
 * @returns {boolean} هل وُجدت بطاقةُ هذا الهدف مرسومةً بالفعل؟
 */
function paintWellTarget(targetId) {
  const body = $('[data-well-body]');
  const cards = body ? body.querySelectorAll('.dw-card') : [];
  if (!cards.length) return false;
  let found = false;
  cards.forEach((card) => {
    const on = Boolean(targetId) && card.dataset.dwTarget === targetId;
...
async function renderWells() {
  ...
  /* ⚠️ تُؤخَذ التذكرةُ قبل أوّل `await` — بعدَه يكون غيرُك قد سبقك. */
  const mine = (wellDrawTicket += 1);
  const stale = () => mine !== wellDrawTicket;
  ...
  /*
   * ⚠️ **ونداءٌ سبقَته نقلةٌ أخرى ينصرف هنا صامتًا** — قبل أن يمسّ
   *    وسمًا أو رقمًا. وإلّا كتب أرقامَ هدفٍ غادرتَه فوق أرقام الحاضر.
   */
  if (stale()) return;
  ...
  /* ⚠️ والتركيبُ ينتظر هو الآخر — فيُسأل الأحدثُ مرّةً ثانيةً بعده. */
  if (stale()) return;
  /*
...
function quickVoiceHtml() {
  ...
  return html`
    ...
    <div class="sh-qv-row">
      ...
      <div class="sh-qv-now">
        <span class="sh-qv-lang" data-qv-lang dir="ltr">${currentVoiceLanguage()}</span>
        <button type="button" class="sh-qv-locate" data-sh="qv-locate">📍 اعرض الصوت الحاليّ</button>
      </div>
    </div>
    <!--
...
/**
 * لغةُ الصوت الحاليّ — **إن عُرفت** من بياناتٍ قائمة: قائمةُ متصفّح الأصوات
 * (`getVoices()`) إن جُلبت، أو أصواتُ الجهاز (`ctx.voices`) لمزوّد المتصفّح.
 * ولا تُخمَّن: غيرُ المعروفة فراغ.
 */
function currentVoiceLanguage() {
  const id = activeVoiceName();
  if (!id) return '';
  const hit = browseVoices.find((v) => v.providerId === ctx?.ttsProviderId && v.id === id);
  if (hit?.language) return hit.language;
  if ((ctx?.ttsProviderId || BROWSER_PROVIDER_ID) === BROWSER_PROVIDER_ID) {
    const all = [...(ctx?.voices?.russian || []), ...(ctx?.voices?.others || [])];
    return all.find((v) => v.name === id)?.lang || '';
  }
  return '';
}

/** «الصوت · المزوّد» كما يُعرَض خارج اللوحة (زرُّها، ولوحةُ التسجيل). */
function currentVoiceLabel() {
  const entry = (ctx?.ttsProviders || []).find(({ provider }) => provider.id === ctx?.ttsProviderId);
  return [activeVoiceName() || 'الافتراضيّ', entry?.provider.name].filter(Boolean).join(' · ');
}
...
function paintPreview() {
  ...
  quickVoice.querySelectorAll('[data-sh="qv-preview"], [data-sh="qv-try"]').forEach((btn) => {
    ...
    if (key === 'current') btn.setAttribute('aria-label', on ? 'أوقف التجربة' : 'جرّب الصوت');
      ...
      /* جانبُ الصوت الحاليّ في المقارنة «يُسمَع» لا «يُجرَّب». */
      const verb = btn.closest('.sh-qv-slot.is-current') ? 'اسمع' : 'جرّب';
      btn.setAttribute('aria-label', `${on ? 'أوقف تجربة' : verb} ${btn.dataset.name || ''}`.trim());
    }
  });
}
 ...
 * ══════════════════════════════════════════════════════════════════ */
let browseVoices = [];
/** «ركّز على الصوت الحاليّ» في الرسمة التالية للقائمة — مرّةً لكلّ فتح. */
let focusCurrentPending = false;
...
function paintCompare() {
  ...
  /*
   * ⚠️ **جرّب قبل أن تعتمد** (Voice Center · التكامل مع التدريب): جانبُ الصوت
   *    الحاليّ يحمل «الحاليّ» وزرُّه «اسمع»؛ والآخرُ «جرّب» و«استخدم هذا الصوت».
   *    التجربةُ `qv-try` كما هي — لا تحفظ ولا تمسّ التدريب؛ والاعتمادُ
   *    `pickVoice` نفسُها — فلا حفظَ إلّا بالضغطة الأخيرة.
   */
  const active = activeVoiceName();
  const isCurrent = (v) => v.providerId === ctx.ttsProviderId && v.id === active;
  const currentKey = active ? voiceKey(ctx.ttsProviderId, active) : '';
  const canAddCurrent = currentKey && byKey.has(currentKey) && !voiceCompare.includes(currentKey);
  const slot = (v) => {
    if (!v) {
      return canAddCurrent
        ? '<div class="sh-qv-slot is-empty"><button type="button" class="sh-qv-cmp-cur" data-sh="qv-cmp-current">+ الصوت الحاليّ</button></div>'
        : '<div class="sh-qv-slot is-empty">اختار صوتًا تاني بـ ⇄</div>';
    }
    const canTry = v.providerId === ctx.ttsProviderId;
    const cur = isCurrent(v);
    return html`<div class="sh-qv-slot${cur ? ' is-current' : ''}" data-qv-slot="${voiceKey(v.providerId, v.id)}">
      <span class="sh-qv-slot-txt">
        <span class="sh-qv-name" dir="auto">${v.name}</span>
        <small dir="rtl">${raw(cur ? '<b class="sh-qv-cur">الحاليّ</b> · ' : '')}<bdi>${v.providerName}</bdi>${raw(v.language ? html` · <bdi dir="ltr">${v.language}</bdi>` : '')}</small>
        ${raw(cur ? '' : html`<button type="button" class="sh-qv-use" data-sh="qv-use" data-p="${v.providerId}" data-v="${v.id}"
          aria-label="استخدم هذا الصوت: ${v.name}">✓ استخدمه</button>`)}
      </span>
      <button type="button" class="sh-qv-try" data-sh="qv-try" data-p="${v.providerId}" data-v="${v.id}"
        data-name="${v.name}" aria-label="${cur ? 'اسمع' : 'جرّب'} ${v.name}" aria-pressed="false"
...
function paintVoiceBrowser({ reveal = true } = {}) {
  ...
  /* الصوتُ الحاليّ يُضاء لحظةً حين يُفتَح القسم — والقائمةُ مُرَّرت إليه أعلاه. */
  if (focusCurrentPending && chosen) {
    focusCurrentPending = false;
    chosen.classList.add('is-focus');
    setTimeout(() => chosen.classList.remove('is-focus'), 1600);
  }
  paintVoiceSections(active);
  paintCompare();
  paintPreview();
...
function openQuickVoice() {
  ...
  quickVoice.querySelector('[data-qv-browse]')?.addEventListener('toggle', (event) => {
    ...
    /* ⚠️ ويُركَّز على الصوت الحاليّ: تمريرٌ إليه ووميضٌ قصير — عند كلّ فتح. */
    focusCurrentPending = true;
    loadVoiceMarks().then(loadVoiceBrowser);
  }, wired({}, panelSignal()));
...
function paintVoiceShortcut() {
  const btn = document.querySelector('[data-sh="qv"]');
  if (!btn || !ctx) return;
  const label = `الصوت: ${currentVoiceLabel()}`;
  btn.setAttribute('aria-label', label);
  btn.title = label;
}
...
function syncQuickVoice() {
  ...
  const lang = quickVoice.querySelector('[data-qv-lang]');
  if (lang) lang.textContent = currentVoiceLanguage();
 ...
 *    أو اختفى) — فنطاقٌ ميّتٌ بينهما يمتصّ الاهتزاز (بند 14).
 *
 * ═══════════════════════════════════════════════════════════════
 * WS48 — والرقمان صارا **مقيسين** لا مُختارَين
 * ═══════════════════════════════════════════════════════════════
 *
 * ⚠️ كانا `900/924`، وكان `924` أعلى من الحاجة بنحو مئة بكسل. وقياسُ
 *    `max-content` لكلّ عنقودِ تحكّمٍ في الصفحتين يعطي أضيقَ عرضٍ لا
 *    ينكسر فيه شيء:
 *
 *      اليمنى (الكون)  · ترانسبورت 268 · أوضاع 181 → 268 + حشو 88 = 356
 *      اليسرى (الورقة) · تبويبات   309              → 309 + حشو 68 = 377
 *      الكعب                                                        =  24
 *
 *    وبالنسبة الافتراضية 55/45 يصير القيدُ الحاكم هو اليمنى — إذ
 *    تأخذ 45٪ فقط ممّا بعد الكعب:
 *
 *      أدنى كتابٍ مريح = 356 ÷ 0.45 + 24 ≈ **815px**
 *
 *    فالتحوّلُ إلى «واحدة» تحت **820**، والعودةُ إلى «اثنتين» عند
 *    **860** — أربعون بكسلًا نطاقًا ميّتًا.
 *
 * ⚠️ **ولا يكفي الرقمُ وحدَه: كان العرضُ نفسُه غيرَ مطّرد.** سكّةُ
 *    التنقّل كانت تظهر عند 900px فتأكل 86px، فيهبط الكتابُ من 875
 *    إلى 790 بتكبير النافذة بكسلًا واحدًا — أي أن التكبير كان
 *    **يُصغّر** الكتاب. وقد رُفع حدُّ ظهورها إلى 930 في `shadow.css`
 *    ليبقى الكتابُ فوق 820 لحظةَ ظهورها، فلا قفزةَ عكسيّة تُعيدك
 *    إلى صفحةٍ واحدةٍ وأنت توسّع. الشرحُ الكاملُ هناك.
 */
const BOOK_SINGLE_BELOW = 820;
const BOOK_TWO_ABOVE = 860;
...
function applyFonts() {
   ...
   *
   * ⚠️ **بلاغُك**: «أبدّل الخطَّ فلا يتغيّر النصُّ الكاملُ في الصفحة».
   *
   * وقِيس حيًّا على ٤١٢×٩١٥ وخطُّ القراءة `philosopher`:
   *
   *     البطل   Philosopher
   *     الرقاقة Philosopher
   *     «نصّ كامل» (`.sh-flow-s`)  **Inter**  ← خطُّ التطبيق، يتبع لا شيء
   *
   * والسببُ من عائلة ما أُصلح في الطور الثاني: `applyFonts` تعدّ
   * أربعةَ أسماء — البطل، وسطورَ النصّ، ولوحةَ الأصل، والرقائق —
   * و«نصٌّ كامل» ليس أحدَها. فوُلد بلا أبٍ يُورِثه خطًّا.
   *
   * ⚠️ **ويتبع `font` لا `fontDoc` — والقرارُ مقيسٌ لا مُرجَّح.**
   *    ربطتُه أوّلًا بـ`fontDoc` بحجّة أنّ «نصّ كامل» و«جمل» وجهان
   *    لنصٍّ واحد. ثمّ قُرئ زرُّ Aa الذي تضغطه: اسمُه «جرّب خطوطًا
   *    للجملة» وهو يفتح المُنتقي على صفحة **المسرح**، فيكتب `fontId`.
   *    فربطُه بـ`fontDoc` كان سيترك بلاغَك قائمًا بحرفه: تضغط Aa
   *    فلا يتغيّر النصُّ الكامل.
   *
   *    ولوحةُ الأصل (`.sh-origin-*`) تتبع `font` منذ الطور الثاني
   *    بطلبك صراحةً — فهذه ثالثةُ كتلةٍ روسيّةٍ في الصفحة تتبع
   *    **خطَّ القراءة المعتمَد**، والنمطُ صار هو القاعدة.
   *
   * ⚠️ **وسطورُ «جمل» تبقى على `fontDoc`** كما كانت: لها صفحتُها في
   *    المُنتقي الكامل، ولم يطلب أحدٌ نقضَها. وهو تفاوتٌ **مُعلَنٌ**
   *    لا مُخفًى — والوجهان يلتقيان متى ساويتَ الإعدادين.
   *
   * ⚠️ **ولا إعدادَ ثانٍ أُنشئ**: نفسُ `fontId` ونفسُ المُنتقي ونفسُ
   *    الكاتب — سطرٌ يصل ما كان مقطوعًا، لا حالةٌ جديدة.
   */
  /*
   * ══════════ وتَبِعَ «نصٌّ كامل» أخاه «جمل» أخيرًا (WS-FFP) ══════════
   *
   * ⚠️ **وهذا نقضٌ لاختياري في WS-VFP — بدليلك أنت.** ربطتُه يومَها
   *    بـ`ctx.font` (خطّ المسرح) على فرضٍ كتبتُه بيدي: «زرُّ Aa الذي
   *    تضغطه يفتح المُنتقيَ على صفحة المسرح». والفرضُ كان خطأً: أنت
   *    تضغط شارةَ Aa **في صفحة المحتوى** (`fontChip('doc')`)، وهي تكتب
   *    `fontDocId` — ولذلك كان «جمل» يستجيب و«نصّ كامل» لا.
   *
   *    وقِيس في جلسةٍ واحدة (١٢٨٠×٨٠٠، نفسُ النصّ، نفسُ الاختيار):
   *
   *        المختار  · جمل                     · نصّ كامل
   *        caveat   · Caveat  (يرسم الروسيّة) · Philosopher (لا يرسمها)
   *        marck    · Marck Script (يرسمها)   · Philosopher (لا يرسمها)
   *        system   · system-ui (يرسمها)      · Philosopher (لا يرسمها)
   *
   *    و«نصّ كامل» و«جمل» **وجهان لنفس النصّ في نفس الصفحة**، فمن
   *    الحقّ أن يتبعا نفسَ الإعداد. فصار `ruDoc` — نفسُ ما يتبعه أخوه.
   *
   * ⚠️ **ولم يُنشَأ إعدادٌ ثالث**: `fontDocId` قائمٌ منذ WS36، والمُنتقي
   *    واحدٌ، والكاتبُ واحد. تغيّر السلكُ لا عددُ الحالات.
   */
...
function openMyVoice() {
  ...
  return openVoiceAttempts(
    ...
    /* صوتُ التعلّم الحاليّ مرجعًا — نفسُ ما ينطق به «المرجع» (Voice Center). */
    { referenceVoice: currentVoiceLabel() },
  );
}
...
function paintPhrase() {
  ...
  if (lbl) {
    ...
    /*
     * ⚠️ **والشريطُ يحمل النبر كما تحمله الرقائق** (بند ٦).
     *
     *    كان يعرض `phraseText()` خامًا: الرقائقُ فوقه تقول
     *    «по́лностью» والشريطُ تحتها يقول «полностью»، ونسخُ المقطع
     *    يعطي المعلَّم. ثلاثةُ تمثيلاتٍ لشيءٍ واحدٍ باثنتين منها
     *    متّفقتين وواحدةٍ شاذّة — وهو بالضبط ما يمنعه البند: «كلُّ
     *    التمثيلات الحاليّة تستعمل الصيغةَ المعلَّمة الجديدة».
     *
     *    و`markedForCopy` هي البابُ لأنها تحترم مفتاحَ إظهار النبر:
     *    مَن أطفأه يرى الخامَ في الشريط والنسخِ معًا.
     */
    lbl.textContent = hasPhrase()
      ? `مقطع: ${markedForCopy(phraseText())}`
 ...
 *
 * ══════════════════════════════════════════════════════════════════
 * ⚠️ **وهذا الطريقُ كان خارجَ كلّ تنسيق** (WS-VC1A · العطب ١)
 * ══════════════════════════════════════════════════════════════════
 *
 * جردُ V0.1 وجده: نطقُ الكلمة والمقطع — والمرجعُ في لوحة التسجيل —
 * كلُّها تمرّ من هنا مباشرةً إلى `ttsSpeaker.speak`، **بلا مرورٍ
 * بالمحرّك وبلا مطالبةٍ بالناقل**. وأثرُه اتّجاهان:
 *
 *   · لا يُسكِت أحدًا — تلمس كلمةً وتسجيلٌ محفوظٌ يعمل، فيتكلّمان معًا.
 *   · ولا يُسكَت إلّا مصادفةً — إن كان المحرّكُ هو المالكَ وقتَها،
 *     لأنّ `player.pause()` تصل إلى `canceler()` عرَضًا. وإن لم يكن
 *     (وهو حالُ لمسِ كلمةٍ بلا تشغيل) فلا شيءَ يوقفه إطلاقًا — ومن
 *     هنا تسرّب صوتُ المرجع إلى الميكروفون.
 *
 * ⚠️ **ولا يُحَلّ بتمريره على محرّك التكرار**: نطقُ الكلمة فعلٌ مستقلٌّ
 *    لا دورةُ تدريب — تمريرُه هناك يخلط عدَّ التكرار وتقدّمَ الجلسة
 *    بفعلٍ عابر. فالحلُّ أن يصير **مالكًا** للصوت كغيره: يُسكِت ويُسكَت،
 *    ودلالتُه كما هي.
 */
/** اسمُ هذا المالك في الناقل — نطقُ كلمةٍ أو مقطعٍ أو مرجع. */
const SPEAK_OWNER = 'speak';

/**
 * تذكرةُ النطق المباشر — تمنع البائتَ من إفساد الحاليّ.
 *
 * ⚠️ رقمٌ لا رايةٌ منطقيّة: بطلبين متتابعين تُلغي رايةُ الثاني رايةَ
 *    الأوّل فيعود الأوّلُ حيًّا. وهو الدرسُ نفسُه المكتوب في
 *    `speaker-adapter.js` — وهناك يحرس التوليد، وهنا يحرس الحلقةَ
 *    والتحرير.
 */
let speakTicket = 0;
...
async function speakScope(text, { times = 1, voiceName } = {}) {
  ...
  /*
   * ⚠️ **وطلبٌ جديدٌ يُبطل القديمَ صراحةً** (`times` قد تكون أكثر من
   *    واحدة، فالحلقةُ قد تكون في منتصفها حين تُلمَس كلمةٌ أخرى).
   *    والإلغاءُ **قبل** المطالبة: لو تُرك للناقل لَما وقع أصلًا —
   *    راجع الشرحَ تحت.
   */
  speakTicket += 1;
  const myTicket = speakTicket;
  ttsSpeaker?.cancel?.();

  /*
   * ⚠️ **ولا يُسكِت المالكُ نفسَه**: `claimAudio` لا تنادي `silence`
   *    إلّا إن اختلف المُطالِبُ عن المالك (راجع `audio-bus.js`) — فنقرةٌ
   *    ثانيةٌ على كلمةٍ لا تُلغي نفسَها عبر ردّ النداء. ولذلك كان
   *    الإلغاءُ أعلاه صريحًا.
   *
   * ⚠️ **والتذكرةُ تُرفَع في ردّ النداء أيضًا**: مالكٌ جديدٌ (تسجيلٌ،
   *    تشغيلُ جلسة، تسجيلٌ محفوظ) يُسكِتنا، فالحلقةُ يجب أن تعرف أنها
   *    بائتةٌ ولا تُكمل تكرارَها التالي.
   */
  claimAudio(SPEAK_OWNER, () => {
    speakTicket += 1;
    ttsSpeaker?.cancel?.();
  });
  ...
  for (let i = 0; i < Math.max(1, times); i += 1) {
    ...
    /*
     * ⚠️ **وبائتٌ لا يُحرّر ملكيّةَ غيره.** لو عاد نطقٌ قديمٌ بعد أن
     *    صار المالكُ غيرَه، فتحريرُه يُسكِت الجديدَ — وهو العطبُ الذي
     *    يصنعه كلُّ تحريرٍ بلا شرط. فيخرج صامتًا.
     */
    if (myTicket !== speakTicket) return;
    if (out && out.ok === false && out.reason !== 'aborted') break;
  }

  /*
   * ⚠️ **والتحريرُ مشروطٌ بالاسم وبالتذكرة معًا**: `releaseAudio(id)`
   *    تتحقّق من الاسم (فلا نُسكِت مالكًا آخر)، والتذكرةُ تتحقّق من
   *    **أيّ** نطقٍ منّا هو الأخير (فنقرتان متتاليتان لا تُحرّر أولاهما
   *    ملكيّةَ ثانيتهما وهي ما تزال تتكلّم).
   */
  if (myTicket === speakTicket) releaseAudio(SPEAK_OWNER);
}
...
function wireInteractions(main) {
  ...
  main.addEventListener('click', async (event) => {
    ...
    /*
     * ═══════════════════════════════════════════════════════════════
     * ⚠️ **إغلاقُ السكّة بلمسةٍ خارجها — بيدِ مُعالِجٍ لا بحجابٍ أصمّ**
     * ═══════════════════════════════════════════════════════════════
     *
     * كان `.sh-scrim` يبتلع كلَّ لمسةٍ فوق الكتاب ما دامت السكّةُ
     * مفتوحة. وذلك يُغلقها فعلًا — لكنّه **يمنع أيضًا لمسَ رقائق
     * الكلمات**، فتصير أدواتُ الكلمة (اسمع · احفظ · معناها · النطق)
     * متعذّرةً كلّما كانت السكّةُ مفتوحة.
     *
     * ⚠️ **والوصفُ الأوّلُ للعطب كان «أثناء تدريب المسودّة» — وهو خطأ.**
     *    قِيس بأربع حالاتٍ لا اثنتين: أصليّ/تدريب × مفتوحة/مغلقة.
     *    فالفاصلُ **حالُ السكّة لا نوعُ المقطع**؛ وإنّما بدا كذلك لأنّ
     *    السكّةَ تُفتَح افتراضًا حين يبدأ تدريبُ المسودّة.
     *
     * فالقاعدةُ صارت صريحة:
     *
     *   رقاقةُ كلمة  → تمرّ، و`wireChips` تفتح سياقَ الكلمة (السكّةُ تبقى)
     *   ما عداها     → تُغلَق السكّةُ وتُبتلَع اللمسةُ — كما كان الحجاب
     *
     * ⚠️ **ولا يُغلَق شيءٌ من داخل السكّة نفسِها**: أزرارُ الأدوات
     *    واللوحةُ فوقها، ولمسُها عملٌ لا خروج.
     *
     * ⚠️ **والتحكّمُ مستثنًى — انحدارٌ أحدثتُه ثمّ أمسكتُه**: أوّلُ
     *    كتابةٍ لهذا الحارس أغلقت السكّةَ عند لمس «التالي» فلم ينقّل
     *    (`rail:false` والهدفُ لم يتغيّر). و`.sh-transport` و`.sh-modes`
     *    مرفوعان فوق الحجاب بـ`z-index: 12` عمدًا (WS31 ثمّ WS44) —
     *    فالحارسُ يحاكي ما كان الحجابُ يغطّيه لا أكثر.
     */
    /*
     * ⚠️ **و`.sh-toolrail` صار `.sh-edge` (WS-TOOLS-LAYOUT · بند ٤).**
     *    السكّةُ رُفعت، فمُنتقيها هنا كان سيمرّ صامتًا — والمُنتقي الميّتُ
     *    في **حارسٍ** أسوأُ من الميّت في نمط: لا يُخطئ بصوتٍ عالٍ، بل
     *    يُغلِق اللوحَ تحت إصبعك حين تلمس «تعلّم» أو «Аа» وهو مفتوح.
     */
    if (rail.open
      && !event.target.closest('.sh-edge')
      && !event.target.closest('.sh-panel')
      && !event.target.closest('.sh-transport')
    ...
    switch (btn.dataset.sh) {
      ...
      case 'qv-cmp-close':
        return closeCompare();
      case 'qv-cmp-current': {
        /* الصوتُ الحاليّ جانبًا أوّلَ — مقارنةٌ به لا بغيره. */
        const key = voiceKey(ctx.ttsProviderId, activeVoiceName());
        voiceCompare = [key, ...voiceCompare.filter((k) => k !== key)].slice(0, 2);
        return paintVoiceBrowser({ reveal: false });
      }
      case 'qv-use':
        /* ⚠️ الاعتمادُ وحده يحفظ — بنفس مسار اختيار البطاقة. */
        return pickVoice(btn.dataset.p, btn.dataset.v);
      case 'qv-locate': {
        /* 📍 القسمُ الصحيح يُفتَح (والأكورديونُ يطوي الباقي)، ثمّ التركيز. */
        const voices = quickVoice?.querySelector('[data-qv-section="voices"]');
        if (!voices) return undefined;
        if (voices.open) {
          focusCurrentPending = true;
          return paintVoiceBrowser();
        }
        voices.querySelector(':scope > summary').click();
        return undefined;
      }
      case 'qv-cache-clear':
