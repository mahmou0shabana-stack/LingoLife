...
export async function openDraft(kind, text, where = {}) {
  ...
  return createDraft(kind, text, where);
}

/**
 * يُنشئ مسودّةً جديدةً **دائمًا** — ولو كان لنفس النصّ مسودّةٌ أخرى.
 *
 * ═══════════════════════════════════════════════════════════════
 * ⚠️ **ولمَ يحتاج أحدٌ نسختين لنفس النصّ؟**
 * ═══════════════════════════════════════════════════════════════
 *
 * لأنّ النصَّ ليس الهُويّة (WS-SC1). `Хорошо́.` في الجملة ٢ و`Хорошо́.`
 * في الجملة ٤ جملتان مختلفتان في نصٍّ واحد، ولكلٍّ سياقُها الذي
 * تُذاكره. و`openDraft` تُرجع الأولى للثانية — وهو صوابٌ حين كان
 * المفتاحُ نصًّا، وخطأٌ الآن: تكتب في مسودّةِ جملةٍ أخرى وأنت تظنّها
 * مسودّتَك، وهو بعينه ما يمنعه البندُ ٧.
 *
 * ⚠️ **ولا تُنادَ إلّا حين تكون الأخرى مربوطةً بجملةٍ أخرى ربطًا
 *    ثابتًا** — أي حين نعرف يقينًا أنّها ليست هذه. وإلّا فـ`openDraft`
 *    هي البابُ: نسختان بلا سببٍ تشتيتٌ لا دقّة.
 */
export async function createDraft(kind, text, where = {}) {
  const key = subjectKey(text);
  if (!key) throw new Error('مفيش جملة ولا كلمة نفتحلها مسودّة');
