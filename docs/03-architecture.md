...
6. **الترقية لا تستورد من طبقة الخدمات.** الخدمة تتغيّر مع الميزات،
   والترقية المنشورة لا يتغيّر سلوكها أبدًا — فاستيرادها من الخدمة
   يجعل ما كتبته ترقيةُ الأمس يتبدّل كلما عُدِّلت الخدمة اليوم. ما
   تحتاجه الترقيات من ثوابت يعيش في `js/db/seeds.js`، وما تحتاجه من
   دوالّ تكتبه لنفسها (انظر `simpleNormalize` في `migrations.js`).

### مثالٌ عملي: كيف نُقلت الأنواع بلا لمس بياناتك (v7)

الأنواع كانت كتلة JSON في `settings['scene.types']`، وصارت مستودعًا.
وهي أوّل ترقية **تنقل معنًى قائمًا** بدل أن تضيف بجانبه — فتستحقّ أن
تُقرأ كنموذج:

| الخطوة | لماذا |
|---|---|
| بذر المدمجة **بمعرّفاتها الحالية نفسها** (`meeting`…) | `scene.type` يحملها أصلًا، فبمجرّد وجودها في المستودع يصير كل مشهد موصولًا بنوعه — **بلا كتابة بايت في `scenes`** |
| نسخ تخصيصاتك فوق المبذور بنفس المعرّف | تعديلك على «فحص» يبقى تعديلًا عليه لا نوعًا ثانيًا |
| `settings['scene.types']` **لا يُحذف** | شبكة أمان للرجوع (بند 107). لو انكشف عطبٌ بعد أسبوع، تخصيصاتك حيث كانت |
| `scene.eventTypeId` يُملأ من `scene.type`، **والحقلان يُكتبان معًا** | قاعدة 2 أعلاه: القارئ القديم والجديد كلاهما يجد قيمة صحيحة، فيُحذف الأوّل في دورةٍ لاحقة |
| البذر بـ`put` لا `add` | ترقيةٌ أُعيد تشغيلها (استرجاع نسخة، تبديل شرائح) تكون محايدة لا منفجرة |
