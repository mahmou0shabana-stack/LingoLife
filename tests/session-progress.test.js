...
describe('WS-PR · حرّاسٌ على الشاشة', () => {
  ...
  it('١٩ · والموضعُ من النافذة لا من الفهرس المطلق — بعد نزع العدّاد المكرَّر', async () => {
    /*
     * ⚠️ **وهذا الحارسُ كان يشترط العدّادَ القديم بحرفه** — وقد نُزع
     *    ذلك العدّادُ من رأس المسرح (WS-RUE · ٥) لأنّه كان يقول ما
     *    يقوله سطرُ التقدّم تحته وشريطُ القاع. فلا يُشطَب الحارسُ ولا
     *    يُترَك يحرس نصًّا ميّتًا: **ما كان يحرسه معنًى لا سطر** — أن
     *    يكون الموضعُ المعروضُ نسبةً إلى نافذة المصدر لا إلى المصفوفة
     *    كلِّها (قِيس قبل إصلاحه: «4 / 03» ثمّ «7 / 03»). وبيتُ ذلك
     *    المعنى الآن `sessionProgress` وسطرُ `.sh-prog-pos`.
     */
    const src = await view();
    /*
     * ⚠️ **ويُقرأ الوسمُ بلا تعليقاته** — وهذا الدرسُ مدفوعُ الثمن في
     *    الحارس ٢٠ تحته بحرفه: كتبتُ في مكان العدّاد المنزوع تعليقًا
     *    يشرح **لِمَ** نُزع، وفيه كلمةُ SENTENCES نفسُها — فسقط الحارسُ
     *    على شرحي لا على شاشة. حارسٌ يقرأ النثرَ يحرس الكتابة.
     */
    const bare = (t) => t.replace(/<!--[\s\S]*?-->/g, ' ').replace(/\/\*[\s\S]*?\*\//g, ' ');
    /* ١) المكرَّرُ ذهب — لا مقامَ في الرأس ولا كلمةُ SENTENCES فيه. */
    const top = bare(src.slice(src.indexOf('class="sh-stage-top"'),
      src.indexOf('data-prog')));
    expect(`مقامٌ في الرأس: ${top.includes('data-pos-total')}`).toBe('مقامٌ في الرأس: false');
    expect(`SENTENCES في الرأس: ${top.includes('SENTENCES')}`).toBe('SENTENCES في الرأس: false');
    /* ٢) والحالةُ باقيةٌ في مكانها — لا فراغَ مكانَ المنزوع. */
    expect(`الحالةُ باقية: ${top.includes('data-status')}`).toBe('الحالةُ باقية: true');
    /* ٣) والموضعُ ما زال يُحسَب من النافذة ويُعرَض. */
    expect(`الموضعُ من النافذة: ${src.includes('sessionProgress(')}`)
      .toBe('الموضعُ من النافذة: true');
    expect(`ويُعرَض: ${/sh-prog-pos[\s\S]{0,60}at\.position/.test(src)}`)
      .toBe('ويُعرَض: true');
  });
