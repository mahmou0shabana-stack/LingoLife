...
describe('وصف المصدر', () => {
  ...
  it('نوعٌ مجهول لا يكسر الوصف', () => {
    /*
     * ⚠️ **وكانت «مصدر» فصارت «نصّ» (WS-I · بند ١).**
     *
     *    هذه القيمةُ تُرسَم نصًّا ظاهرًا على شاشة التدريب
     *    (`shadow-view.js` — `<b>${source.kind}</b>`)، والبندُ ١ يقضي
     *    برفع لفظِ «المصدر» عن الشاشة. فالتبديلُ مقصود، والاختبارُ
     *    يلحق به — لا العكس.
     *
     *    والمقصودُ من الحالة لم يتغيّر: نوعٌ لا نعرفه **لا يكسر الوصف**
     *    ولا يُظهر `undefined` — يقع على الافتراضيّ ويكمل العدّ.
     */
    const d = describeSource({ sourceType: 'somethingNew' }, [seg()]);
    expect(d.kind).toBe('نصّ');
