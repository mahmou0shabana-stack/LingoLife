 ...
 *
 * ═══════════════════════════════════════════════════════════════
 * ⚠️ **والسجلُّ الذي بيدك يُشفى معه — عطبٌ قِستُه في المتصفّح**
 * ═══════════════════════════════════════════════════════════════
 *
 * الكتابةُ وحدَها لا تكفي. شاشةُ الظلّ لا تقرأ المسودّةَ من القاعدة في
 * كلّ رسمة: `resolveDraft` تُعيد **لقطةً محفوظةً في `material`** جُمعت
 * مرّةً عند تحميل الجلسة. فكانت الدورةُ:
 *
 *     رسمة ١ → `storedTargets` = []  → تُولَد معرّفاتٌ → تُكتَب في القاعدة
 *     رسمة ٢ → نفسُ اللقطةِ الجامدة → `[]` مرّةً أخرى → **معرّفاتٌ جديدة**
 *
 * فالمعرّفُ الذي وُعد بأن «يُولَد مرّةً ويعيش» كان يُولَد في كلّ لمسة.
 * وقِستُه: `DT_…6C68_*` ثمّ `DT_…6D3X_*` بعد ضغطةٍ واحدة. وأثرُه ثلاثة:
 *
 ...
 *
 * فتُكتَب القيمةُ في السجلّ الذي بيدنا أيضًا: كلُّ من يمسك بهذا المرجع
 * — و`material` أوّلُهم — يرى ما تراه القاعدة. ولا يُصلحه `readDrafted`
 * وحدَه لأنّها لا تُنادى إلّا بعد حفظِ حالة، لا بعد كلّ فتحة.
 *
...
export async function ensureTargetIds(draft, parsed) {
  ...
  if (changed) {
    await studyDrafts.update(draft.id, { [TARGET_IDS]: targets });
    draft[TARGET_IDS] = targets;
  }
  return targets;
}
