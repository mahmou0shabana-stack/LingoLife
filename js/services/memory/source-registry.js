...
/**
 * يسجّل أيَّ بصمةٍ **غادرت الجهاز** في حزمةٍ نُسخت.
 *
 * ═══════════════════════════════════════════════════════════════
 * ⚠️ **إيصالُ الإرسال — وبلا مخزنٍ ثالثٍ ولا ترقية**
 * ═══════════════════════════════════════════════════════════════
 *
 * حين تعود نتيجةُ التحليل، نحتاج أن نعرف **أيَّ نصٍّ قرأه التحليلُ
 * بالضبط** لا أيَّ نصٍّ في القاعدة الآن. والفرقُ يقع كلَّ مرّةٍ تعدّل
 * فيها نصًّا أثناء الجولة: لو سجّلنا البصمةَ الحاليّة لَبدا تعديلُك
 * محلَّلًا وهو لم يُرسَل قطّ.
 *
 * والبديلُ المرفوض: أن يعيد التحليلُ البصماتِ إلينا. فبصمةٌ يقولها
 * الطرفُ الآخر ادّعاءٌ لا نملك التحقّقَ منه — وهو بالضبط ما يمنعه
 * `import-v2.js` في كلّ شيءٍ آخر.
 *
 * فالإيصالُ يُكتَب هنا، على الصفّ نفسِه، **لحظةَ النسخ** — لأن النسخَ
 * هو الفعلُ الذي تغادر به الحزمةُ الجهازَ فعلًا. والبناءُ وحدَه لا
 * يكفي: قد تبني ثم تغلق الشاشة.
 *
 * ⚠️ وهو **ليس** `markAnalyzed`: «أُرسِل» ≠ «حُلِّل». والحالةُ المعروضةُ
 *    لا تتغيّر بالنسخ، فنصٌّ نسختَه ولم تحلّله يبقى «جديدًا» ويُقترَح.
 */
export async function markSent(entries = []) {
  let written = 0;
  const at = Date.now();
  for (const { key, hash } of entries) {
    ...
    if (!row || row.sentHash === hash) continue;
    /* eslint-disable-next-line no-await-in-loop */
    await memorySources.update(key, { sentHash: hash, sentAt: at });
    written += 1;
  }
  return written;
}
