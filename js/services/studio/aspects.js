...
export const ASPECTS = Object.freeze([
  ...
  {
    /*
     * ⚠️ **هذا هو اختبار السجلّ نفسه.** أُضيف النصّ الأصلي في WS10،
     *    فصار على مئات الذكريات القديمة فارغًا — وإضافتُه هنا
     *    **إعلانٌ واحد**، بلا سطرٍ في شاشة الاستوديو ولا في خدمتها.
     *    وهو ما وُعِد به في docs/13 §١٣٫٢، وهذه أوّل مرّة يُوفَّى فيها.
     */
    id: 'rawTranscript',
    label: 'النصّ الأصلي',
    why: 'اللي اتقال فعلًا بأخطائه. عليه بيقوم «خطأ/طبيعي» والتحليل وحياة التعبير.',
    since: 'WS10',
    fill: FILL.SET,
    /*
     * ⚠️ **ولا يُملأ دفعةً** — لنفس سبب السياق بالضبط، وأشدّ: نصٌّ
     *    واحدٌ على أربعين ذكرى ليس «ما قيل» في أيٍّ منها. وهو فوق ذلك
     *    **يُقفل بأوّل كتابة** (بند 27)، فدفعةٌ خاطئة لا تُصلَح.
     */
    bulk: false,
    bulkReason: 'النصّ الأصلي هو اللي اتقال في اللحظة دي بالذات — ونصّ واحد على أربعين ذكرى مش «اللي اتقال» في ولا واحدة فيهم. وكمان بيتقفل بأول كتابة فمفيش تراجع',
    input: { kind: 'none' },

    missing: (scene, world) => !world.rawByScene?.has(scene.id),
    current: (scene, world) => (world.rawByScene?.get(scene.id) ? 'مكتوب' : ''),
    evidence: (scene, world) => {
      const spoke = speakersOf(scene, world)
        .map((id) => world.personName.get(id))
        .filter(Boolean);
      return spoke.length ? [{ icon: 'chat', text: `اتكلّم هنا: ${spoke.join('، ')}` }] : [];
    },
  },
