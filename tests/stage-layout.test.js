...
describe('WS-VFP · أصغرُ وأشفُّ بلا أن يصغر حبرٌ', () => {
  ...
  it('٦٩ · و«نصٌّ كامل» يتلقّى خطَّ القراءة المعتمَد', async () => {
     ...
     *
     * ⚠️ **وكان هنا «يتبع `ctx.font` لا `ctx.fontDoc`» — وكان خطأً
     *    (WS-FFP).** بنيتُه على فرضٍ كتبتُه بيدي: «زرُّ Aa يفتح المُنتقيَ
     *    على صفحة المسرح». والمستخدم يضغط شارةَ Aa في **صفحة المحتوى**،
     *    وهي تكتب `fontDocId`. فبقي البلاغُ قائمًا طورًا كاملًا، وحرسَه
     *    هذا السطرُ وهو مقلوب.
     *
     *    والدرسُ: **حارسٌ يُثبّت فرضًا لم يُقَس يحرس الخطأَ نفسَه.**
     *    فصار المحروسُ أنّ الوجهين يلتقيان (`ruDoc` لكليهما)، ومعه في
     *    `fulltext-font-parity.test.js` قياسُ عرضٍ **مرسوم** لا اسمَ
     *    عائلة — لأنّ الاسمَ وحدَه هو ما خدعني هنا.
     *
     ...
     *    يتبع **خطَّ الصفحة** (`ctx.fontDoc`) كما يتبعه أخوه «جمل»،
     *    وأنّ ما يُطبَّق هو ما يقدر على الروسيّة (`russianFontId`).
     */
    expect(`بخطّ الصفحة: ${/const ruDoc = russianFontId\(ctx\.fontDoc\)/.test(fonts)}`)
      .toBe('بخطّ الصفحة: true');
    ...
    /* وسطورُ «جمل» على خطّ الصفحة كما كانت — وهي الشاهدُ الذي قيس عليه. */
