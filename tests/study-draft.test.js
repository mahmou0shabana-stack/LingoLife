...
describe('الإعدادات · كل زرّ له معالِجٌ واحد', () => {
  ...
  it('⚠️ الحارس: كل اسم فعلٍ يُصدره الرسم له `case` في المُوزِّع', async () => {
    ...
    const ELSEWHERE = {
      ...
      'qv-provider': 'قائمة select لمصدر الصوت في لوحة الصوت السريعة (Voice Center V1.0C) — حدثُها change فتُعالَج في مستمع change وتنادي setTTSProvider نفسَها',
      ...
      'fix-practice': 'زرّ «تدرّب على الصح» داخل نافذة ذاكرة الكلمة — موصولٌ في onMount الخاصّ بها، لأن مُوزِّع main لا يسمع نقرةً داخل نافذةٍ مُلحَقةٍ بـbody',
      'occ-context': 'زرّ «سياق المرّة دي» داخل نفس نافذة الذاكرة — نفس السبب، ونافذتُه تعلوها ولا تغلقها',
    };
  ...
  it('⚠️ ومفتاحُ الأوضاع على الشاشة لا في لوحة', () => {
     ...
     *
     * ⚠️ **ثمّ رجعت ثلاثةً في WS-FINAL-VISUAL — والنقصُ هو الإصلاح.**
     *    والتعليقُ فوق يقول «يعرفه المحرّك»، وقد جُرِّد المحرّكُ فلم
     *    يعرفه: `PRACTICE_MODE.CONTINUOUS` لا تُقرأ فيه ولا مرّةً
     *    واحدة، والمنطوقُ والأحداثُ والفهرسُ في «متّصل» هي في «جملة»
     *    حرفًا بحرف (والشاهدُ الموجَبُ وضعُ الكلمة: يفترق).
     *
     *    فالذي كان يمشي على الجمل بلا تكرارٍ حقلٌ آخرُ اسمُه `repeatMode`
     *    في مركز التدريب — وهو قائمٌ لم يُمَسّ. ونقلُ الوضع إلى المفتاح
     *    كان صحيحًا يومَه؛ والخطأُ أنّه نُقل ولم يُسأل عمّا يفعل.
     */
...
describe('التحكّم · لا زرَّ تحت لوحة', () => {
  ...
  it('⚠️ والحجابُ لا يبتلع لمسةَ التشغيل على الشاشات العريضة أيضًا', async () => {
     ...
     */
    /*
     * ⚠️ **ولا `lastIndexOf` هنا — وقد سقط هذا الحارسُ به مرّةً** (WS-VPOLISH).
     *    كان يأخذ **آخرَ** كتلةِ `min-width: 900px` في الورقة ويفتّش
     *    فيها وحدَها. فلمّا أُضيفت كتلةٌ عريضةٌ جديدةٌ في آخر الورقة
     *    لغرضٍ آخر (حشوةُ البطل) صار «الآخِرُ» غيرَ المقصود، فسقط
     *    الحارسُ وقاعدةُ الحجاب لم تُمَسّ بحرف.
     *
     *    وهو نفسُ الدرس المكتوب في الحارس الذي فوقه: يُفتَّش عن
     *    **القاعدة** لا عن موضعٍ في ملفّ. فالبحثُ الآن في كلّ كتلةٍ
     *    عريضةٍ حتى تُوجَد.
     *
     *    وقِيس السلوكُ نفسُه حيًّا يومَ سقط الحارس، والسكّةُ مفتوحة:
     *      ١٢٨٠×٨٠٠  الحجاب ٦٠..٦٠٠ والتشغيل ٦٦٤..٧٢٨ — لا تراكب
     *      ٨٢٠×١١٨٠  الحجاب ٥٤..٨٧٦ والتشغيل ١٠٢٦..١٠٩٠ — لا تراكب
     *    و`elementFromPoint` على مركز التشغيل يعيد `i.sh-ico-play`
     *    في المقاسين: الزرُّ يملك لمستَه.
     */
    const wides = [...css.matchAll(/@media \(min-width: 900px\)/g)].map((m) => m.index);
    expect(wides.length > 0).toBe(true);
    /*
     * ⚠️ **وضعفٌ ثانٍ كان فيه وقد شُدّ**: كان يقنع بوجود `bottom:` أيًّا
     *    كانت قيمتُه — فطفرةٌ تُعيده إلى `bottom: 0` (وهو العطبُ الأصليُّ
     *    بحرفه: حجابٌ يغطّي الترانسبورت) كانت تمرّ. فصار يُقرأ **المقدار**
     *    ويُشترَط رفعُه عن القاع.
     *
     *    ⚠️ والعريضُ يرفع بمقدارٍ مكتوب (150px) لا بـ`var(--sh-controls)`
     *       — ذاك للضيّق وحدَه. وقد ظننتُهما واحدًا فشددتُ الحارسَ على
     *       القيمة الخطأ فسقط على ورقةٍ سليمة.
     */
    let lift = '';
    for (const at of wides) {
      const m = /\.sh-scrim \{ bottom: ([^;]+);/.exec(css.slice(at, at + 400));
      if (m) { lift = m[1].trim(); break; }
    }
    const lifted = Boolean(lift) && !/^0(px)?$/.test(lift);
    expect(`رفعُ الحجاب «${lift}» مرفوعٌ ${lifted}`).toBe(`رفعُ الحجاب «${lift}» مرفوعٌ true`);
  });
});
