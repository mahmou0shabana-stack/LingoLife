...
export function parseDraftV2(text) {
  ...
  /*
   * ═══════════════════════════════════════════════════════════════
   * ⚠️ **والسؤالُ يسبق جوابَه دائمًا — ولو كان جوابُه القلبَ نفسَه**
   * ═══════════════════════════════════════════════════════════════
   *
   * بلاغُك (WS-RUE · ٣): «اقرأ كلَّ سؤالٍ مع إجابته وحدةً واحدة».
   * وقِيس ما كان يحدث فعلًا قبل أيّ تغيير — بناطقٍ بديلٍ يسجّل ما
   * يُطلَب نطقُه، على مسودّةٍ فيها نصُّك بحرفه:
   *
   *     «перейти́ к документа́ции»  ← ×٥   (القلب = الإجابة)
   *     «перейти к чему?»          ← ×٣   (السؤال، **بعدها**)
   *
   * أي أنّك تسمع الإجابةَ ثمّ سؤالَها. والسببُ بنيويٌّ لا سهو:
   * `Ответ:` في هذه المسودّات **يعيد نصَّ القلب نفسِه**، فلا تُبنى له
   * وحدةُ إجابةٍ (وذلك صحيحٌ — وإلّا تضاعف النصُّ وحدتين)، فيبقى
   * السؤالُ معلَّقًا بعد قلبِه لأنّ المؤلّفَ كتبه بعده في الورقة.
   *
   * **فالقاعدةُ تصير: موضعُ السؤال يتبع موضعَ إجابته لا موضعَ كتابته.**
   * وإجابتُه إمّا وحدةُ `RECALL_ANSWER` تليه، وإمّا القلبُ نفسُه —
   * وفي الحالة الثانية يُقدَّم السؤالُ على القلب.
   *
   * ⚠️ **و`pairLead` وصفٌ لا إعداد**: يقول «الوحدةُ التي تليني هي
   *    جوابي». يقرؤه المحرّكُ فينطق السؤالَ **مرّةً** ثمّ يمضي إلى
   *    جوابه بلا وقفة — فيصيران وحدةَ تعلّمٍ واحدة. ولا يُحفَظ في
   *    القاعدة ولا يدخل بصمةَ الهدف: هو مشتقٌّ من البنية في كلّ قراءة.
   */
  /*
   * ⚠️ **و«الإجابةُ هي القلبُ» تُقاس بلا ترقيمٍ ولا حرفٍ كبير.**
   *    `subjectKey` تُسقط النبرَ وتُوحّد الحالة، وتُبقي النقطةَ — وهي
   *    بابُ هذا العطب: المؤلّفُ يكتب القلبَ «перейти к документации»
   *    والإجابةَ «Перейти к документации.»، فيقرؤهما المحلّلُ نصّين
   *    مختلفين فيبني لهما **وحدتين**، فتسمع النصَّ نفسَه مرّتين —
   *    مرّةً قبل سؤاله ومرّةً بعده. وهو عينُ ما يمنعه التعليقُ المكتوبُ
   *    تحت («ذلك يضاعف النصَّ نفسَه وحدتين»)، والترقيمُ وحدَه كان يعبر
   *    من تحته.
   *
   * ⚠️ **ولا تُمَسّ `subjectKey` نفسُها**: هي مادّةُ بصمات الأهداف،
   *    وتوسيعُها يغيّر كلَّ معرّفٍ مسكوكٍ في قاعدة المستخدم. فالتسامحُ
   *    **محلّيٌّ لهذه المقارنة وحدَها**.
   */
  const bare = (t) => subjectKey(t).replace(/^[«"'([]+|[.,!?;:…»"')\]]+$/g, '').trim();
  const sameSubject = (a, b) => Boolean(a) && Boolean(b) && bare(a) === bare(b);

  const recallUnits = (one, { before }) => {
    const out = [];
    for (const pair of one.pairs) {
      const selfReply = sameSubject(pair.reply, one.ru);
      /* جوابُه القلبُ ⇒ موضعُه قبل القلب، أيًّا كان موضعُ كتابته. */
      const wantsBefore = selfReply ? true : Boolean(pair.before);
      if (wantsBefore !== before) continue;
      if (pair.cue) {
        out.push({
          ...blank(), role: ROLE.RECALL_CUE, ru: pair.cue, ar: pair.cueAr,
          parent: one.ru, family: one.family,
          /* جوابُه يليه: وحدةُ إجابةٍ مستقلّة، أو القلبُ نفسُه. */
          pairLead: Boolean(pair.reply),
        });
      }
      if (pair.reply && !selfReply) {
        out.push({
