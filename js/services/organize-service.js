...
async function resequence(parentId) {
  ...
  for (const { link: row } of rows) {
    ...
    /*
     * ═══════════════════════════════════════════════════════════════
     * ⚠️ **وتُكتَب كلُّ الحوافّ ولو لم يتغيّر رقمُها** (WS-G، بند ٢٥)
     * ═══════════════════════════════════════════════════════════════
     *
     * كان هنا `if (row.order !== n)` — تحسينٌ صحيحٌ لجهازٍ واحدٍ
     * وكاسرٌ لجهازين. لأن **الترتيبَ خاصّةُ قائمةٍ لا خاصّةُ حافّة**:
     * تحريكُ عنصرٍ واحدٍ يعيد تعريفَ مواضعِ الجميع.
     *
     * وحين يكتب الجهازُ حافّتين فقط، تصل الجارَ **نصفُ قائمة**، فلا
     * يستطيع أن يعرف ترتيبَ من لم يُذكَر — فيخلط رقمًا من هنا برقمٍ
     * من هناك ويُنتج ترتيبًا ثالثًا لم يطلبه أحد. وهذا بعينه ما قِيس:
     * الأساسُ `أ ب ج`، وهذا رتّبه `ب أ ج`، وذاك `أ ج ب`، فكان الناتج
     * `أ ب ج` — أي **لا أحد**.
     *
     * فكتابةُ القائمة كوحدةٍ تجعل الحزمةَ تحمل الترتيبَ كاملًا، ويصير
     * «استعمل ترتيبَه» قابلًا للتنفيذ فعلًا.
     *
     * والثمنُ: `n` كتابةً بدل ٢ عند إعادة ترتيبٍ يدويّة. وهي عمليّةٌ
     * نادرةٌ يقوم بها إصبعٌ، والأبناءُ عشراتٌ لا آلاف.
     */
    /* eslint-disable-next-line no-await-in-loop -- حافّةٌ بعد حافّة، والقائمةُ قصيرة */
    await relationships.update(row.id, { order: n });
  }
}
