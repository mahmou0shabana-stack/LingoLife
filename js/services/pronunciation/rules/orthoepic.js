...
registerRule({
  id: 'RU_CLUSTER_ZHCH_SHCH',
  category: RULE_CATEGORY.ASSIMILATION,
  stage: STAGE.ORTHOEPIC_REWRITE,
  /* ⚠️ بعد ٣٤٠ مباشرةً — نفسُ العائلة، ونتيجتُها نفسُ الحرف. */
  priority: 341,
  summary: 'жч/шч تُنطَقان صوتًا واحدًا طويلًا ليّنًا [ɕː] كأختَيهما сч/зч',
  explain: '«жч» بتتنطق زيّ «щ» — «мужчина» بتتقال «мущина».',
  source: 'oross.ruslang.ru §6 + orfogrammka «Буквенные сочетания сч, зч, жч, шч»: '
    + '«звук [ш\':] передаётся … буквосочетаниями зч, сч, жч, шч, здч, стч» — مثالاها مужчина وперебежчик',
  /*
   * ⚠️ **مبدئيّةٌ لا مُتحقَّقة — والسببُ في الوصول لا في اللغة.**
   *
   *    الظاهرةُ نفسُها لا خلافَ عليها، وهي **العائلةُ عينُها** التي
   *    تُنفّذها `RU_CLUSTER_SCH_ZCH` بدليلٍ من جامعة موسكو. لكنّ
   *    صفحتَي المصدر أعلاه **محجوبتان عن هذه البيئة** (بوّابةُ الخروج
   *    ترفض `oross.ruslang.ru` و`orfogrammka.ru`)، فما وصلني فهرسُ
   *    بحثٍ يقتبسهما لا نصُّهما. ومقتطفٌ من فهرسٍ ليس مقتطفًا من مصدر.
   *
   *    فالقاعدةُ تُنفَّذ لأن البند ٥ن يطلبها والمنهجَ يوجبها، وتُعلَّم
   *    `PROVISIONAL` لأنّي **لم أقرأ الصفحةَ بعينـي**. ومتى فُتح
   *    الوصولُ تُرفَع إلى `VERIFIED` بسطرٍ واحد.
   */
  status: STATUS.PROVISIONAL,
  evidence: EVIDENCE.SNIPPET,
  /*
   * ⚠️ **ولولا هذه القاعدةِ لَخرجت `мужчина` خطأً صامتًا مزدوجًا:**
   *    `RU_REGRESSIVE_DEVOICING` كانت تهمس `ж` قبل `ч` فتُنتج
   *    «мушчина» — نطقٌ لا يقوله روسيّ، ولا يُسقِط اختبارًا لأن كلَّ
   *    قاعدةٍ فيه على حدة صحيحة. والعنقودُ يُحلّ **قبل** الأصوات، فلا
   *    يبقى لـ`ж` وجودٌ تُهمَس فيه.
   */
  applies: (word) => /[жш]ч/.test(word),
  transform: (word) => swap(word, /[жш]ч/g, () => 'щ'),
});
